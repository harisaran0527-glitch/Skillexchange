...
const connectMongoDB = async () => {
  ...
  if (!uri || uri.trim() === '' || uri === 'your_mongodb_atlas_connection_string') {
    console.error('\n========================================================================')
    console.error('[DB] CRITICAL ERROR: MongoDB connection URI is not configured!')
  ...
  if (uri.includes('localhost') || uri.includes('127.0.0.1') || uri.includes('::1')) {
    console.error('\n========================================================================')
    console.error('[DB] CRITICAL ERROR: Localhost / loopback database addresses are forbidden!')
  ...
  if (!uri.startsWith('mongodb://') && !uri.startsWith('mongodb+srv://')) {
    console.error('\n========================================================================')
    console.error('[DB] CRITICAL ERROR: Invalid connection string scheme!')
  ...
  try {
    ...
    if (error.message.includes('Authentication failed') || error.message.includes('auth failed') || error.code === 8000) {
      console.error('[DB] REASON: Authentication failed. The password in your connection string is likely incorrect.')
